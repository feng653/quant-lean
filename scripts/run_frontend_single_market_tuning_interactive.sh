...
config=${QUANT_TUNING_CONFIG:-$repo/scripts/frontend_tuning/non_ml_single_market_tuning.v2.json}
artifact=${QUANT_TUNING_ARTIFACT_DIR:-$HOME/.local/state/quant-platform/frontend-tuning/non-ml-single-csi300-cross-validated-20260731}
...
cd "$repo" || exit 72
print "量化平台 122 次真实行情单策略前端实验：请输入现有账号。密码不会显示、写入文件或命令参数。"
...
export QUANT_TUNING_CONFIG=$config
...
run_with_bounded_resume() {
  ...
  while (( attempt <= max_resume_attempts )); do
    ...
    delay=$(( 5 * (2 ** (attempt - 1)) ))
    (( delay > 30 )) && delay=30
    print -u2 "$label 遇到已确认的瞬态错误，${delay} 秒后从 checkpoint 自动续跑（$attempt/$max_resume_attempts）。"
    sleep "$delay"
    attempt=$((attempt + 1))
  done
}

print "正在进行真实浏览器登录与双源缓存只读预检……"
run_with_bounded_resume --live-preflight "前端预检" || exit $?

print "预检通过，开始 10 基线 + 102 选模 + 10 锁定测试。"
export QUANT_TUNING_EXECUTE_CONFIRM=122_FRONTEND_EXPERIMENTS
run_with_bounded_resume --execute "122 次前端实验" || exit $?
