"""Alpha158 + XGBoost 因子排序策略 —— 多因子训练 + Walk-Forward 预测."""
...
from backend.strategies.ml.runtime import import_xgboost
...
class Alpha158XGBStrategy(TrainableStrategy):
    """Alpha158 因子 + XGBoost 排序策略。

    策略原理:
        与 Alpha158+LGB 共享相同的 Alpha158 因子输入，但使用 XGBoost 作为
        替代模型。XGBoost 采用层级优先（level-wise）的树生长方式，
        配合 L1/L2 正则化和列采样，在小数据集上比 LightGBM 更鲁棒、
        更不容易过拟合。同样采用 Walk-Forward 方式按月重训和预测，
        买入预测排名 Top K% 的股票。
        该策略作为与 LightGBM 版本的对照实验，验证因子和模型各自的重要性。

    适用范围:
        多因子量化选股，特别适合 50-200 只股票的中等规模数据集。
    """
    ...
    def metadata(cls) -> StrategyMetadata:
        return StrategyMetadata(
            ...
            display_name="Alpha158 + XGBoost 因子策略",
            version="1.0.0",
            category=StrategyCategory.ML,
            description=(
                "基于 Alpha158 量化因子集和 XGBoost 极端梯度提升树的机器学习策略。"
                "计算 50+ 个核心因子，用 XGBoost 训练排序模型预测下月收益率。"
                "XGBoost 的层级优先生长策略和 L1/L2 正则化使其在小数据集上更鲁棒。"
                "采用 Walk-Forward 方式每月重新训练，买入预测排名 Top K% 的股票。"
                "作为 LightGBM 版本的对照实验，验证模型选择对因子策略的影响。"
            ),
            ...
            tags=["机器学习", "XGBoost", "Alpha158", "因子投资", "排序学习", "Walk-Forward"],
        )
    ...
    def train(
        ...
        if progress_callback:
            progress_callback(0.30, f"正在训练 XGBoost (样本数: {len(X_train)})...")

        model = self._fit_xgb(X_train, y_train, params)

        if progress_callback:
            progress_callback(0.90, "XGBoost 训练完成，正在计算特征重要性...")
    ...
    def fit(
        ...
        """在指定窗口训练 XGBoost 模型（复用 prepare 缓存的因子）."""
        self.prepare(pivot, params)
        X_train, y_train = self._build_training_matrix(
            self._factor_df,
            pivot,
    ...
    def _fit_xgb(
        ...
        if X_validation is not None and y_validation is not None:
            model_kwargs["early_stopping_rounds"] = max(
                10,
