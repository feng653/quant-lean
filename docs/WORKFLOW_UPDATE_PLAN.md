...
> 生效日期：2026-08-05 v2 ｜ 状态：**临时过渡计划（非宪法）**
> **本文件只指导"下一步"实施**：每个阶段完成即按实际更新，全部阶段完成（v0.9.0 发布）后**本文件删除**。
> 长期有效的规则在配套宪法文档；本文件不构成宪法，与宪法冲突时以宪法为准。
...
| **协调者**（= 任何在项目根启动的 opencode 会话，自动激活，见 AGENTS.md） | 发布 issue、接受用户要求、分支和版本规划管理、反映工作流状态（读门禁→同步镜像）、向上简报 | 不做用户要求之外的规划、不替代机器门禁判断 |
